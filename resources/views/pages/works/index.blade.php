@extends('layouts.app')
@section('content')
    <section id="works" class="w-full min-h-screen bg-black text-white py-20 px-6 md:px-16 pb-48">
        ...
        <div class="flex flex-wrap gap-6 mb-12 text-lg">
            ...
            @foreach($subcategories as $sub)
                <a href="{{ route('works.subcategory', $sub->slug) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
            @foreach($products as $product)
                <x-work-card 
                    ...
                    title="{{ $product->name }}" 
                    category="{{ strtoupper($product->subCategory->name ?? 'KARYA') }}"
                    :tags="$product->tags ? explode(',', $product->tags) : []"
                    :href="route('works.show', $product->slug)" 
                />
            @endforeach
        </div>
    </section>
